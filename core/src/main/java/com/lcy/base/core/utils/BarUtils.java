...
public final class BarUtils {
    ...
    public static int getStatusBarHeight() {
        Resources resources = CoreApplication.Companion.instance().getResources();
        int resourceId = resources.getIdentifier("status_bar_height", "dimen", "android");
        if (resourceId > 0) {
            return resources.getDimensionPixelSize(resourceId);
        }
        return CommonUtil.dp2px(CoreApplication.instance(), 25);
    }
     ...
     */
    public static int setStatusBarLightMode(@NonNull final Activity activity, final boolean isLightMode) {
        return setStatusBarLightMode(activity.getWindow(), isLightMode);
    }
     ...
     * @param isLightMode True to set status bar light mode, false otherwise.
     */
    private static int setStatusBarLightMode(@NonNull final Window window, final boolean isLightMode) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            return StatusBarMode.KITKAT;
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            return StatusBarMode.M;
        }
        if (MIUISetStatusBarLightMode(window, isLightMode)) {
            return StatusBarMode.MIUI;
        }
        if (FlymeSetStatusBarLightMode(window, isLightMode)) {
            return StatusBarMode.FLYME;
        }
        return StatusBarMode.OTHERWISE;
    }
    ...
    private static boolean MIUISetStatusBarLightMode(Window window, boolean dark) {
        ...
        if (window != null) {
            ...
            try {
                ...
                extraFlagField.invoke(window, dark ? darkModeFlag : 0, darkModeFlag);//状态栏透明且黑色字体
                result = true;
    ...
    public static int getActionBarHeight() {
        ...
        if (CoreApplication.Companion.instance().getTheme().resolveAttribute(android.R.attr.actionBarSize, tv, true)) {
            return TypedValue.complexToDimensionPixelSize(
                    tv.data, CoreApplication.Companion.instance().getResources().getDisplayMetrics()
            );
        }
        return 0;
    }
    ...
    private static void invokePanels(final String methodName) {
        try {
            ...
            Object service = CoreApplication.Companion.instance().getSystemService("statusbar");
            @SuppressLint("PrivateApi")
    ...
    public static int getNavBarHeight() {
        Resources res = CoreApplication.Companion.instance().getResources();
